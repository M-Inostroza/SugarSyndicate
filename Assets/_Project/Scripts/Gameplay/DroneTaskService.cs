...
public class DroneTaskService : MonoBehaviour
{
    ...
    public event System.Action<int> OnCrawlerCountChanged;
    ...
    void SpawnCrawler()
    {
        ...
        OnCrawlerCountChanged?.Invoke(crawlers.Count);
    }
    ...
    bool IsCrawlerTask(DroneTaskTarget task)
    {
        if (task is BlueprintTask bt)
            return bt.Type == BlueprintTask.BlueprintType.Cable && !bt.IsNodeLinkCableBlueprint;
        return false;
    }
