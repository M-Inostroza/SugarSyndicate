    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < runs.Count; i++)
        {
            ...
            else if (outgoing[i].Count == 1)
            {
                ...
                if (headIdx == i)
                {
                    // self-loop: enqueue immediately when possible, otherwise hold in a merger queue
                    for (int k = 0; k < ej.Count; k++)
                    {
                        var item = ej[k];
                        if (!targetRun.TryEnqueue(item.id))
                        {
                            // ensure a merger exists to buffer items waiting to rejoin
                            var me = headEp as MergerEndpoint;
                            if (me == null)
                            {
                                me = new MergerEndpoint();
                                heads[headIdx] = me;
                                headEp = me;
                            }
